#include <linux/kernel.h>
#include <linux/init.h>
#include <linux/module.h>
#include <linux/kdev_t.h>
#include <linux/fs.h>
#include <linux/cdev.h>
#include <linux/device.h>
#include<linux/slab.h>                 //kmalloc()
#include<linux/uaccess.h>              //copy_to/from_user()
#include <linux/kthread.h>             //kernel threads
#include <linux/sched.h>               //task_struct 
#include <linux/delay.h>

//DEFINE_SPINLOCK(etx_spinlock);
spinlock_t etx_spinlock;

unsigned long etx_global_variable=0;

dev_t dev = 0;
//static struct class *dev_class;
static struct cdev etx_cdev;
 
static int __init etx_driver_init(void);
static void __exit etx_driver_exit(void);
 
static struct task_struct *etx_thread1;
static struct task_struct *etx_thread2; 

/*************** Driver functions **********************/
static int etx_open(struct inode *inode, struct file *file);
static int etx_release(struct inode *inode, struct file *file);
static ssize_t etx_read(struct file *filp, char __user *buf, size_t len,loff_t * off);
static ssize_t etx_write(struct file *filp, const char *buf, size_t len, loff_t * off);
 /******************************************************/

int thread_function1(void *pv);
int thread_function2(void *pv);

//thread fun 1
int thread_function1(void *pv)
{
    while(!kthread_should_stop())
    {
        if(!spin_is_locked(&etx_spinlock))
        {
            printk(KERN_ALERT "Spinlock is not locked in thread function1\n");
        }
        etx_global_variable++;
        printk(KERN_ALERT "In thread function1 %lu",etx_global_variable);
        spin_unlock(&etx_spinlock);
        msleep(1000);
    }
    return 0;
}

//thread fun2
int thread_function2(void *pv)
{
    while(!kthread_should_stop())
    {
        spin_lock(&etx_spinlock);
        etx_global_variable++;
        printk(KERN_ALERT "In thread function2 %lu\n",etx_global_variable);
        spin_unlock(&etx_spinlock);
        msleep(1000);
    }
    return 0;
}

//File operation structure  
static struct file_operations fops =
{
    .owner          = THIS_MODULE,
    .read           = etx_read,
    .write          = etx_write,
    .open           = etx_open,
    .release        = etx_release,
};

//sys calls
//open call
static int etx_open(struct inode *inode, struct file *file)
{
    printk(KERN_ALERT "Device file opened....\n");
    return 0;
}
//close call
static int etx_release(struct inode *inode, struct file *file)
{
    printk(KERN_ALERT "device file closed...\n");
    return 0;
}
//read call
static ssize_t etx_read(struct file *filp, char __user *buf, size_t len, loff_t *off)
{
    printk(KERN_ALERT "read function\n");
    return 0;
}
//write
static ssize_t etx_write(struct file *filp, const char __user *buf, size_t len, loff_t *off)
{
    printk(KERN_ALERT "write function\n");
    return len;
}

//module init fun
static int __init etx_driver_init(void)
{
    //allocating major number
    if((alloc_chrdev_region(&dev,0,1,"etx_Dev"))<0)
    {
        printk(KERN_ALERT "cannot allocate major num\n");
        return -1;
    }
    printk(KERN_ALERT "major=%d minor=%d \n",MAJOR(dev),MINOR(dev));

    //create cdev struct
    cdev_init(&etx_cdev,&fops);

    //add char device
    if((cdev_add(&etx_cdev,dev,1))<0)
    {
        printk(KERN_ALERT "cannot add\n");
    }

  /*  //create struct class
    if((dev_class=class_create(THIS_MODULE,"etx_class"))==NULL)
    {
        printk(KERN_ALERT "cannot create class\n");
    }

    //create device
    if((device_create(dev_class,NULL,dev,NULL,"etx_device"))==NULL)
    {
        printk(KERN_ALERT "cannot create device\n");
    }*/

    //create thread1
    etx_thread1=kthread_run(thread_function1,NULL,"eTx Thread1");
    if(etx_thread1)
    {
        pr_err("kthread1 created...\n");
    }
    else{
        pr_err("cannot created...\n");
    }

    //create thread2
    etx_thread2=kthread_run(thread_function2,NULL,"eTx Thread2");
    if(etx_thread2){
        pr_err("kthead2 created....\n");
    }
    else{
        pr_err("cannot created....\n");
    }
    spin_lock_init(&etx_spinlock);

    printk(KERN_ALERT "device driver insert doneeee....\n");
    return 0;


}

//module exit fun
static void __exit etx_driver_exit(void)
{
    kthread_stop(etx_thread1);
    kthread_stop(etx_thread2);
    //device_destroy(dev_class,dev);
    //class_destroy(dev_class);
    cdev_del(&etx_cdev);
    unregister_chrdev_region(dev, 1);
    printk(KERN_ALERT "Device Driver Remove...Done!!\n");
}

module_init(etx_driver_init);
module_exit(etx_driver_exit);

MODULE_LICENSE("GPL");
//MODULE_AUTHOR("EmbeTronicX <embetronicx@gmail.com>");
MODULE_DESCRIPTION("A simple device driver - Spinlock");